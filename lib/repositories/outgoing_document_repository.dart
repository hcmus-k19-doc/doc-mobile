...
class OutgoingDocumentRepository extends BaseRepository {
  ...
  Future<OutgoingDocument> getOutgoingDocumentDetailById(int id) async {
    try {
      final response =
          await provider.get(url: "/$id", cancelToken: cancelToken);
  ...
  Future<List<ProcessingDetail>> getIncomingDocumentProcessByI(int id) async {
    try {
      final response = await provider.get(
          url: "/$id/processing-details?onlyAssignee=true",
          cancelToken: cancelToken);

      return List<ProcessingDetail>.from(
          response.map((e) => ProcessingDetail.fromJson(e))).toList();
    } catch (err) {
      rethrow;
    }
  }
  ...
  Future<PaginationOutgoingDocument> fetchPaginationOutgoingDocumentList(
    ...
    try {
      ...
      tempPagination.listOutgoingDocument = List<CustomOutgoingDoc>.from(
          l.map((e) => CustomOutgoingDoc.fromJson(e))).toList();
